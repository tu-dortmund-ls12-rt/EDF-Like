    ...
    for k in range(n-1,-1,-1):
        ...
        for j in range(n):
            ...
            if j == k: # this is the j==0 case
                ...
                for ji in range(n):
                    if ji != k and Atilde[ji] <= Atilde[j]:
                        R += min(math.floor(htasks[k]['period']/htasks[ji]['period']), math.ceil((htasks[k]['period'] - mjk)/htasks[ji]['period']))*htasks[ji]['execution']
                for ji in range(n):
                    if ji != k and Atilde[ji] > Atilde[j]:
                        R += min(math.floor(htasks[k]['period']/htasks[ji]['period'])+1.0, math.ceil((htasks[k]['period'] - mjk)/htasks[ji]['period']))*htasks[ji]['execution']
                R+= mjk
                Rtildek[j] = R
        Rtilde[k] = min(Rtildek)
        # print('Rtilde')
